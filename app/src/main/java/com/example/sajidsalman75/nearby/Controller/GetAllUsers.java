...
public class GetAllUsers extends AppCompatActivity {
    ...
    Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            if(res.getCount() == 0) {
                ...
                adapter = new UserAdapter(GetAllUsers.this,list);
                lv.setAdapter(adapter);
                lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    @Override
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(GetAllUsers.this);
                        ...
                        prefs.edit().putString("role1", list[position].getROLE()).commit();
                        prefs.edit().putInt("userid", list[position].getID()).commit();
                        Intent i = new Intent(GetAllUsers.this, AddAdmin.class);
                        flag = true;
                        startActivity(i);
                    }
                });
    ...
    protected void onStart(){
        super.onStart();
        prefs = PreferenceManager.getDefaultSharedPreferences(GetAllUsers.this);
        boolean isLogin = prefs.getBoolean("Islogin", false);
        if (!isLogin){
            Intent i = new Intent(GetAllUsers.this, Login.class);
            startActivity(i);
        }
        if (flag){
            res = db.getAllUsers();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        progress = new ProgressDialog(GetAllUsers.this);
